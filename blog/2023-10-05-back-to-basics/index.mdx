...
significantly improve either code readability and our efficiency. Let's jump into them!
...
We could fix it by wrapping math into braces:
...
Also called conditional operator. Its purpose is to simplify some `if`-statement-based code allowing us to achieve the same in single expression.

Let's define a goal for an exercise: to check if value was provided or not, applying fancy label.
...
That's quite important thing as it can reverse your code logic in some (if not most) cases.

:::

There are 2 ways to check equality of things strict and weak one.
...
We will use helper function to see how it behaves in different scenarios (including one that proves weak comparison to be useful, stay tuned!).
...
That's why weak comparison is discouraged. In fact, JS converts the values first in the case.

Think of the example as of array that has `0` elements and simple `0` numeric value.
The result would be the same for blank string, even containing a white spaces:
...
This can be simplified though, thanks to weak comparison against nullish value:
...
Have you ever faced a problem to get some properties of an object, ignoring the rest?
...
We were told to create application header including the name and environment.
...
but destructed shape might be easier to use and more readable though:
...
How to get all the properties of an object but a few?
...
But how the code would look like if there were tens of properties? It would be enormous spaghetti!
...
One of common cases with handling data is that something is missing, and we want to apply a default value.

JS got nice thing a while ago to handle that - we can just use `someValue ?? defaultValue` expression.
...
That may be handy in some cases, though in mostly we need to ensure that value is not nullish.
...
Pretty similar to [nullish coalescing operator](#nullish-coalescing-operator) in its behavior but with slightly different purpose.
It helps in property/variable initialization when it's nullish to not be anymore.
...
Sometimes we need to assign a property to an object only at particular circumstances
while having `undefined` or `null` as a value is not an option. We can take [spread operator](#spread-operator) into account:
...
So it doesn't work just like that. What else can we do? Extra [ternary operator](#ternary-operator) is a common way.
...
I think we can agree with that we're dealing with nullish values pretty often.
Usually we're trying to access a property on an object that doesn't really exist.

In case we're reaching top level property that may not be a nightmare to make simple if statement and exit early.
But things used to be much more complicated than that.

Let's say we have blob storage of documents in cloud and many persons has access to edit it while the first one is initial creator.
We want to display somewhere information about the second contributor.

In ideal world everything is easy, and we'd have accessor like this one:
...
Unfortunately we don't have later contributors returned separately, rather than there's collection of all of them.

We've got a requirement from Product Owner to display `-` in case there's only one contributor.
...
It's pretty neat when using along with [nullish coalescing operator](#nullish-coalescing-operator)

## `in` operator

Sometimes we need to check if there's particular property in an object.
...
By checking if such object has particular property:
...
Last but not least. It's rarely used in repositories though likely it could be present at local code, not pushed yet.

Comma expressions does everything from left to right but returns only last part.

It's like CSV would return last column.
...
That probably wasn't best use case but keeping how it behaves in mind, you could debug things easily inline:
