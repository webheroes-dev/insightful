...
function NaiveButtonCounter() {
  ...
  return (
      <>
        <button type='button' onClick={() => incrementCounter()}>Click me to increase counter!</button>
...
Looking into the console after pressing the button three times will show:
...
This is because the variable `count` is not tracked by React, and hence the component is not rerendered when its value is updated.
...
Now looking into the console after three button presses will show:
...
This is also essential if a single function calls the `setState` function multiple times.
...
function ButtonCounter() {
  ...
  return (
      <>
        <button type='button' onClick={() => incrementCounter()}>Click me to increase counter by {count}!</button>
        <p>Times pressed: {count}</p>
      </>
  );
}
...
If line 8 had instead been `setCount(count + 1)`, then the counter would only increase by 1 on each button press.
